	...
	while (Game == true)
	{
		...
		if(PieceMoveChoice <= Move)
		{
			...
			if(DiceSelect == 1)
			{
				...
				if(WhosTurn == 1)
				{
                    for(i = 0; i<CombinedResults+1; i++)
                    {
					    if(BETWEEN(Colsj[HorizontalMovement],0,13)) //Left Hand Side
					    {
						    if(BETWEEN(Rowsi[VerticalMovement],0,7)) //Quad 1
						    {
							    HorizontalMovement++;
								HorizontalMovement++;
						    ...
						    if(BETWEEN(Rowsi[VerticalMovement],8,13)) //Quad 3
						    {
							
							   HorizontalMovement+=2;
                               if(HorizontalMovement >= 12)
                               {
                                HorizontalMovement = HorizontalMovement+3;
                               }
						    }
					    }
					    if(BETWEEN(Colsj[HorizontalMovement],15,26)) //Right Hand Side
					    {
						    if(BETWEEN(Rowsi[VerticalMovement],0,7)) //Quad 2
						    {
							    HorizontalMovement+=-2;
                               if(HorizontalMovement <=15)
                               {
                                HorizontalMovement=HorizontalMovement-3;
                               }
						    }
						    if(BETWEEN(Rowsi[VerticalMovement],8,13)) //Quad 4
						    {
							    HorizontalMovement+=2;
                                if(HorizontalMovement>=26)
                                {
                                    HorizontalMovement = 26;
                                }
						    }
					    } 
                    }
